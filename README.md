...
If a tag-triggered release fails because the workflow needs a fix, merge the fix into `main` and retry the existing tag with `gh workflow run release.yml --ref main -f tag=vMAJOR.MINOR.PATCH`. The manual run uses the updated workflow but tests and packages the unchanged tag commit. It requires an existing version tag on `main` and does not create or move tags.
